...
The dev environment is configured with mysql as the backend service.
```sh
cd ~/path/to/application/
docker-compose up -d --build

# initiate and create database
...
### Access mysql db
```$ docker-compose exec db mysql -uflask_boilerplate_username -pflask_boilerplate_password```
